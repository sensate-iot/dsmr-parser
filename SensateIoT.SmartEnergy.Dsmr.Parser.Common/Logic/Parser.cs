	...
	public class Parser : IParser
	{
		...
		private static IEnumerable<string> GetPropertiesWithKey(string key)
		{
			...
			foreach(var property in GetTelegramProperties()) {
				var attr = property.GetCustomAttributes(typeof(ObisAttribute), false).Cast<ObisAttribute>()
					.FirstOrDefault();
		...
		private static void SetTelegramProperties(ref Telegram telegram, ICollection<string> properties,
		...
		private static void SetPropertyValue(ref Telegram telegram, PropertyInfo propertyInfo, string value,
		                                     string obisValueUnit = null)
		{
			...
			if(propertyInfo.Name == nameof(Telegram.GasConsumption)) {
				telegram.HasGasConsumption = true;
			}
		}
			...
			if(converterAttribute != null) {
				...
				if(converterType == null) {
					throw new InvalidOperationException(
						$"Unable to determine converter type for: {converterAttribute.ConverterTypeName}");
				}
